...
public class ProcessorConfig {
        ...
        public ItemProcessor<BillingSourceRow, BillingWriteItem> billingProcessor(
                ...
                return row -> {
                        ...
                        // 계약 할인
                        List<DiscountInfo> contractDiscounts = contractDiscountService.calculateContractDiscounts(row,
                                        now);
                        discountInfoList.addAll(contractDiscounts);

                        // 생일 할인
                        DiscountInfo birthdayMonthDiscount = eventDiscountService
                                        .birthdayMonthDiscount(memberCredential, billingFee);
                        ...
                        List<DiscountInfo> additionalServiceDiscounts = additionalServiceDiscountService
                                        .calculateAdditionalServiceDiscounts(registration,
                                                        additionalServiceSubscriptions);
                        ...
                        // 장기 고객 할인
                        Optional<DiscountInfo> loyaltyDiscount = loyaltyDiscountService
                                        .calculateLoyaltyDiscount(registration, plan.getBaseFee());
                        ...
                        List<DiscountEntity> discountEntities = discountInfoList.stream()
                                        .map(d -> DiscountEntity.builder()
                                                        .billingId(createdBilling.getId().getId())
